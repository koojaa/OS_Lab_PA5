...
// old => parent process's page table
// new => child process's page table
// sz => parent process's memorys size(bytes)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for (i = 0; i < sz; i += PGSIZE)
  {
    ...
    // get PPN inside of PTE (offset not relected)
    pa = PTE2PA(*pte);

    // clear PTE_W bit in the PTE both for parent and child
    flags = PTE_FLAGS(*pte);
    flags &= (~PTE_W);

    // change parent pagetable's entry with modified permission bit
    // only change the page table entry without freeing the actual physical frame
    ...
    // copy parent pagetable to the child

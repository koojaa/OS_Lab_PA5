...
void usertrap(void)
{
  ...
  else if (r_scause() == 15)
  {
    ...
    // get 4KB aligned virtual address which triggered store page fault
    ...
    if (refCount > 1)
    {
      // copy memory
      char *mem = kalloc();
      memmove(mem, (char *)paddr, PGSIZE);

      // change page table entry with writable flag
      flags |= PTE_W;
